...
struct resized_arena_cord      // Find the co-ordinates 
{
 ...
 Mat a_arena=imread("arena_obj.png",1);
 vector<Point> pnt;
        ...
        if(rank_arena[i]==0)
        {
            for(int j=0;j<subregions_template.size();j++)
            {
                ...
                if(template_match(subregions_arena[i],subregions_template[j]))
                {
                    ...
                    cout<<"i ="<<i<<"j ="<<j<<endl;
                    imshow(to_string(i)+"arena",subregions_arena[i]);
                    namedWindow(to_string(j)+"template",WINDOW_NORMAL);
                    imshow(to_string(j)+"template",subregions_template[j]);
...
void color_detect(Rect rect_i,int i_temp)
 {
    ...
    for(int i=(rect_i.tl()-u).y;i<=(rect_i.br()+u).y;i++)
    {
        ...
        for(int k=0;k<5;k++)
            if(abs(B-given_BGR[k][0])<=3 && abs(G-given_BGR[k][1])<=3 && abs(R-given_BGR[k][2])<=3)
                {
...
cout<<"k"<<k+1<<endl;
 ...
 vector<Mat> template_process()
 {
	...
	imwrite(to_string(i)+"t.png",a(roi));        
	}
    return subregions;
 }
...
/*
const char* arduino_file="/dev/ttyACM1";
const char* command_set="Command_Set.txt";
*/
/*
void send_command()
{
    ...
    ofstream ofs;
    ifstream ifs;
    ifs.open(command_set);
    ofs.open(arduino_file);
    ifs>>ch;
    ofs<<ch;
    ifs.close();    
    ofs.close();
